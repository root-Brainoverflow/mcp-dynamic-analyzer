...
| Bootstrap planner | `infrastructure/bootstrap.py` + `recipes.py` | ✅ Preflight (로컬 manifest / npm view / PyPI API) → Recipe 매칭 → 파생 이미지 빌드 + 캐시. 로컬 manifest면 선언된 deps를 `pip`/`npm install`하는 generic 액션 항상 추가 (`_local_install_action`) |

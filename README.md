...
6. [Syscall 추적](#syscall-추적)
7. [설정 파일 레퍼런스](#설정-파일-레퍼런스)
8. [출력 형식](#출력-형식)
9. [구현 상태](#구현-상태)
10. [요구사항](#요구사항)
...
| 네트워크 모니터 | `infrastructure/netmon.py` | ✅ 오케스트레이터 통합 (`network.log_all_traffic` 또는 `mode=allowlist` 시 자동 시작) |
...
| 컴포넌트 | 파일 | 상태 |
|---|---|---|
| R1: 데이터 접근 | `scanners/r1_data_access.py` | ✅ 동작 (Docker+strace 시 file_read/write까지 커버, 로컬 fallback은 file_open만) |
| R2: 코드 실행 | `scanners/r2_code_exec.py` | ✅ 동작 (Docker+strace 시 execve, 로컬 fallback은 자식 프로세스 폴링) |
| R3: LLM 조작 | `scanners/r3_llm_manipulation.py` | ✅ 즉시 동작 (기본: metadata/content poisoning, opt-in: embedded LLM fuzzing) |
...
| Chain Attack | `scanners/chain_attack.py` | ✅ 즉시 동작 (server metadata 기반) |
